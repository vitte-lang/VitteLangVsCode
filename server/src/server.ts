...
function getDocumentSettings(resource: string): Thenable<ServerSettings> {
  ...
  if (!r) { r = connection.workspace.getConfiguration({ scopeUri: resource, section: "vitte" }) as Thenable<ServerSettings>; documentSettings.set(resource, r); }
  return r;
}
...
connection.onInitialize((params: InitializeParams): InitializeResult => {
  ...
  if (hasWorkspaceFolderCapability) init.capabilities.workspace = { workspaceFolders: { supported: true, changeNotifications: true } };
  return init;
});
...
connection.onInitialized(() => {
  ...
  if (hasWorkspaceFolderCapability) {
    connection.workspace.onDidChangeWorkspaceFolders((e) => {
      workspaceFolders = e.added.length ? e.added : workspaceFolders?.filter((wf) => !e.removed.find((r) => r.uri === wf.uri)) ?? null;
    });
  }
});

connection.onDidChangeConfiguration(() => { documentSettings.clear(); documents.all().forEach(validateTextDocument); });
...
async function validateTextDocument(textDocument: TextDocument): Promise<void> {
  ...
  let problems = 0;
  const pushDiag = (d: Diagnostic) => { diagnostics.push(d); problems++; return problems < settings.maxNumberOfProblems; };
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const m of matchAllRx(DIAG_RE_TODO, line)) {
      ...
      if (!pushDiag({ severity: DiagnosticSeverity.Warning, message: `Marqueur ${m[1]} détecté — à traiter.`, range: Range.create(Position.create(i, start), Position.create(i, start + m[1].length)), source: "vitte-lsp" })) break;
    }
    if (problems >= settings.maxNumberOfProblems) break;

    for (const m of matchAllRx(DIAG_RE_QUESTION, line)) {
      const start = m.index ?? 0;
      if (!pushDiag({ severity: DiagnosticSeverity.Error, message: `Séquence "???" détectée — remplace par du vrai code ou un TODO.`, range: Range.create(Position.create(i, start), Position.create(i, start + m[0].length)), source: "vitte-lsp" })) break;
    }
    if (problems >= settings.maxNumberOfProblems) break;

    const tw = line.match(DIAG_RE_TRAILING_WS);
    if (tw && tw.index is not None): pass
    if (tw && tw.index !== undefined) {
      if (!pushDiag({ severity: DiagnosticSeverity.Hint, message: "Espace(s) superflu(s) en fin de ligne.", range: Range.create(Position.create(i, tw.index), Position.create(i, line.length)), source: "vitte-lsp", tags: [DiagnosticTag.Unnecessary] })) break;
    }
    if (problems >= settings.maxNumberOfProblems) break;

    if (line.length > settings.lineLengthLimit) {
      pushDiag({ severity: DiagnosticSeverity.Information, message: `Ligne longue (${line.length} > ${settings.lineLengthLimit}).`, range: Range.create(Position.create(i, settings.lineLengthLimit), Position.create(i, line.length)), source: "vitte-lsp" });
    }
    if (problems >= settings.maxNumberOfProblems) break;
  }
...
const KEYWORDS = ["module","use","pub","fn","struct","enum","trait","impl","let","const","match","if","else","while","for","loop","return"];

connection.onCompletion((_params): CompletionItem[] => {
  const doc = documents.get(_params.textDocument.uri);
  const items: CompletionItem[] = [];
  for (const kw of KEYWORDS) items.push({ label: kw, kind: CompletionItemKind.Keyword, detail: "mot-clé Vitte" });
  if (doc) {
    for (const s of extractSymbols(doc)) {
      items.push({ label: s.name,
        kind: s.kind === SymbolKind.Function ? CompletionItemKind.Function :
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const [rx, kind, nameSel] of patterns) {
      for (const m of matchAllRx(rx, line)) {
        ...
        symbols.push({ name, kind, range: Range.create(start, end), selectionRange: Range.create(start, end), children: [] });
      }
    }
  }
  return symbols;
...
connection.languages.semanticTokens.on((params: SemanticTokensParams) => {
  ...
  for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
    ...
    if (cmt >= 0) { builder.push(lineIdx, cmt, line.length - cmt, token("comment"), NO_MOD); continue; }
    for (const m of matchAllRx(/"([^"\\]|\\.)*"/g, line)) builder.push(lineIdx, m.index, m[0].length, token("string"), NO_MOD);
    for (const m of matchAllRx(/\b\d(_?\d)*(\.\d(_?\d)*)?\b/g, line)) builder.push(lineIdx, m.index, m[0].length, token("number"), NO_MOD);
    for (const kw of KEYWORDS) for (const m of matchAllRx(new RegExp(`\\b${escapeRx(kw)}\\b`, "g"), line)) builder.push(lineIdx, m.index, m[0].length, token("keyword"), NO_MOD);
    ...
    for (const [rx, kind] of decls) for (const m of matchAllRx(rx, line)) {
      const name = (m as any)[1] ?? (m as any)[2]; if (!name) continue;
...
function getWordAt(doc: TextDocument, pos: Position): string | null {
  const text = doc.getText(); const offset = doc.offsetAt(pos);
  let start = offset, end = offset;
  while (start > 0 && /\w|_/.test(text.charAt(start - 1))) start--;
  while (end < text.length && /\w|_/.test(text.charAt(end))) end++;
  return end > start ? text.slice(start, end) : null;
}
function escapeRx(s: string): string { return s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"); }
function* matchAllRx(rx: RegExp, line: string): Generator<any> {
  let m: RegExpExecArray | null;
  const r = new RegExp(rx.source, rx.flags.includes("g") ? rx.flags : rx.flags + "g");
  while ((m = r.exec(line))) {
    const index = m.index ?? 0;
    yield Object.assign(m, { index });
...
documents.onDidClose((e) => { connection.sendDiagnostics({ uri: e.document.uri, diagnostics: [] }); documentSettings.delete(e.document.uri); });
